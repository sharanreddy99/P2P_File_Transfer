...
/**
 * The PeerController class manages peer connections and file download
 * operations.
 * It provides methods for checking the status of operations, marking file
 * downloads as complete,
 * and retrieving information about connected peers.
 *
 * 
 * Note: This class ensures thread safety for critical operations through
 * synchronization.
 *
 *
...
public class PeerController {
	...
	/**
	 * Sends a HAVE message to all connected peers except the one with the given
	 * peer ID.
	 *
	 * @author Sharan Sai Reddy KOnda
	 * @param index       The index of the piece that the sender now has.
	 * @param givenPeerId The ID of the peer to exclude from receiving the HAVE
	 *                    message.
	 */
	 ...
	 *
	 * @author Sharan Sai Reddy Konda
	 * @param contenderID The ID of the contender peer to be optimistically
	 *                    unchoked.
	 */
	...
	public void optimisticallyUnChokePeersWithContender(String contenderID) {
		...
		// Find the specified contender in the list of peer handlers and send an unchoke
		// message
		for (PeerHandler peerHandler : listPeerHandlers) {
	...
	/**
	 * Handles the choke or unchoke operation for a list of peer IDs based on the
	 * specified type.
	 *
	 * @param listPeerIds A list of peer IDs to be processed.
	 * @param type        The type of message to be sent
	 *                    (Constants.TYPE_CHOKE_MESSAGE or
	 *                    Constants.TYPE_UNCHOKE_MESSAGE).
	 * @author Adithya KNG
	 */
	...
	/**
	 * Updates the list of choked peers and triggers the choke operation for the
	 * specified peer IDs.
	 *
	...
	/**
	 * Inserts a piece into the piece manager, updating the bit field and logging
	 * the progress.
	 * 
	 * @author Adithya KNG
	 ...
	 */
	public synchronized void receiveAndStorePiece(PeerMessage pieceMessage, String sourcePeerID) {
	...
	/**
	 * Generates a PeerMessage containing a data segment (piece) from the specified
	 * index.
	 *
	 * @author Adithya KNG
	 * @param index The index of the data segment (piece) to be included in the
	 *              PeerMessage.
	 * @return A PeerMessage containing the data segment, or null if an exception
	 *         occurs.
	 */
	...
	/**
	 * Informs all other connected peers to terminate the connection as the current
	 * peer
	 * will be shutting down due to task completion.
	 * Marks the current peer as having successfully downloaded the file.
	 * Sends shutdown messages to all other peers, allowing them to request missing
	 * pieces
	...
	/**
	 * Calculates and returns the count of potential new connections based on the
	 * peers with
	 ...
	 * 
	 * @author Adithya KNG
	 * @param id The identifier or id of the peer for which the file download is
	 *           marked as complete.
	 */
