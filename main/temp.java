 ...
 * Peer Handler
 */
public class PeerHandler123 implements Runnable {
	 ...
	 * ObjectOutputStream and starts the PeerMessageSender. Additionally, it proceeds to handle
	 * the initialization of the ChunkRequester and Logger for the PeerHandler.
	 *
	 * @param pc               The PeerController instance associated with the PeerHandler.
	...
	public boolean handlePeerMessageSender(PeerController pc, ObjectOutputStream peerOutputStream) {
		// Initialize the PeerMessageSender using the provided ObjectOutputStream
		peerMessageSender = MessageHelper.returnPeerMessageSender(peerOutputStream);

		// Check if the PeerMessageSender is successfully initialized
		if (peerMessageSender != null) {
			// Start the PeerMessageSender
			startPeerMessageSender(peerMessageSender);
			// Proceed to handle the initialization of the ChunkRequester and Logger
			return this.handleChunkRequesterAndLogger(pc);
		}

		// Close the connection if the PeerMessageSender initialization fails
		return closeStreams();
	}

	/**
	 * Starts a new thread for the PeerMessageSender using the given MessageHelper.
	 *
	...
	public static void startPeerMessageSender(MessageHelper msgHelper) {
		// Start a new thread for the PeerMessageSender using the provided MessageHelper
		new Thread(msgHelper).start();
	}

	/**
	 * Handles the initialization of the ChunkRequester and Logger for the PeerHandler
	...
	public boolean handleChunkRequesterAndLogger(PeerController pc) {
		// Initialize the ChunkRequester using the provided PeerController and PeerHandler
		chunkRequester = NextRequestHelper.getNewInstance(pc, this);
	 ...
	 * managed by the ChunkRequester for subsequent processing.
	 *
	...
	private void unchokeMessage(PeerMessage msg) {
		...
		toggleIsChokedByNeighborPeer(false);
	 ...
	 * This method logs the reception of the choke message, updates the 'isChokedByNeighborPeer' status,
	...
	private void chokeMessage() {
		...
		// Update the 'isChokedByNeighborPeer' status
		toggleIsChokedByNeighborPeer(true);
	}
	...
	private void bitFieldMessage(PeerMessage message) {
		try {
			...
			boolean handshakeProcessDone = isHandshakeReceived && isHandShakeSent;
	...
	private void handleBitFieldMessageProcess() {
		...
		// Synchronize on 'this' to safely update the isChunkStarted flag
		synchronized (this) {
			// Set the flag indicating the start of chunk processing
			this.isChunkStarted = true;
		}
	}
	...
	private void processHandshakeMessage(HandshakeMessage message) {
		...
		if (!isHandShakeSent) {
			...
			if (message.getHeader().equals(Constants.HANDSHAKE_HEADER_STRING) &&
				...
				if (isHandShakeSent && !chunkStatus()) {
					new Thread(chunkRequester).start();
					downloadTime = System.currentTimeMillis();
					downloadSize = 0;
					synchronized(this){
	...
	private void requestMessage(int index) {
		...
		peerMessageSender.sendMessageWithDelay(peerMessage,2000);
	}
	...
	boolean initiateHandShakeMessage() {
		synchronized (this) {
			...
			try {
				// Send the HandshakeMessage to the peer using the PeerMessageSender
	 ...
	 *
	 * This method is responsible for sending a BitFieldMessage to the peer using the PeerMessageSender
	...
	void initiateBitFieldMessage() {
		synchronized(this){
			// Send the BitFieldMessage to the peer using the PeerMessageSender with a delay of 4000 milliseconds
			peerMessageSender.sendMessageWithDelay(controller.getPeerMessage(), 4000);
		}
	}
	 ...
	 * the PeerMessageSender only if the peer is not choked. Any exceptions during
	...
	public void initiateInterestedMessage(PeerMessage msg) {
		try {
			...
			boolean sendMessage = !isChokedByNeighborPeer && peerMessageSender.sendMessage(msg);
	 ...
	 *
	 * This method sends a NotInterestedMessage to the peer using the PeerMessageSender,
	...
	public void initiateNotInterestedMessage(PeerMessage msg) {
		// Send the NotInterestedMessage to the peer using the PeerMessageSender, potentially introducing an error
		peerMessageSender.sendMessageWithError(msg);
	}
	...
	public void initateRequestMessage(PeerMessage msg) {
		...
		boolean sendMessage = !isChokedByNeighborPeer ? peerMessageSender.sendMessageWithError(msg) : false;
	}
	 ...
	 * It sends the Have message using the peerMessageSender and handles any potential errors.
	 *
	...
	public void initiateHaveMessage(PeerMessage message) {
		...
		peerMessageSender.sendMessageWithError(message);
	}
	...
	public double getDownloadSpeed() {
		return (System.currentTimeMillis() - downloadTime) != 0 ? ((downloadSize * 1.0) / ((System.currentTimeMillis() - downloadTime) * 1.0)) : 0;
	}
	...
	public void toggleIsChokedByNeighborPeer(boolean value){
		isChokedByNeighborPeer = value;
	}
